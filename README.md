...
**一次手势只翻一页。** 没有用原生滚动 + `scroll-snap`：那套带惯性，甩一下能连翻
三四页，收不住。改成一条 `transform` 轨道（`#track` / `#vtrack`），位移超过 18%
宽度、或者甩得够快（<260ms 且超过 24px）就走一页，否则弹回，位移上限硬钉在 ±1。
好处不只是手感 —— 位置变成 `atIndex` 一个数，不再需要读写 `scrollLeft`，
重画时也就不会被滚动位置反咬。

`paint()` 是原地更新，节点数不变就只改背景图，不 `innerHTML = ''` 重建。
清空重建会有一瞬间整块是空的（远程图还没加载完），看上去就是白屏一闪。
重画后按**图片 id** 找回位置，不是按下标 —— 列表长度会从 1 变 8，按下标必然被夹到 0，
那正是「设完壁纸自己跳回最新一张」的成因。
